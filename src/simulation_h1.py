...
def run_simulation_h1():
    print("Running 2025 H1 Simulation (Jan-Jun)...")
    ...
    print(f"H1 Results: WinRate={win_rate:.1f}%, Sharpe={sharpe:.2f}")

    # --- Static Chart Generation (Added per request) ---
    print("Saving Static PnL Chart...")
    plt.figure(figsize=(12, 6))
    plt.plot(df['timestamp'], df['cumulative_pnl'], label='Cumulative PnL (€)', color='green')
    plt.title(f"2025 H1 Simulation | Net PnL: €{df['cumulative_pnl'].iloc[-1]:,.2f} | Sharpe: {sharpe:.2f}")
    plt.xlabel("Date")
    plt.ylabel("PnL (€)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    
    out_chart = "output/simulation_h1_chart.png"
    if not os.path.exists("output"): os.makedirs("output")
    plt.savefig(out_chart)
    plt.close() # Close to prevent interference with Animation figure
    print(f"Chart saved to {out_chart}")

    # --- Visualization ---
    # H1 is Hourly Data. Stride 3 = 3 Hours per Frame.
    # Matches H2 (which uses Stride 12 on 15-min data).
    WINDOW_SIZE = 72 
    STRIDE = 3
    ...
    def update(frame_idx):
        ...
        title_top = f"2025 H1 Backtest | Sharpe: {sharpe:.2f} | Win Rate: {win_rate:.1f}%"
    ...
    out_gif = "output/simulation_h1.gif"
    if not os.path.exists("output"): os.makedirs("output")
    
    print(f"Saving H1 GIF to {out_gif}...")
