...
def run_simulation_h2():
    print("Running 2025 H2 Simulation (Jun-Feb)...")
    ...
    print(f"Simulating {len(df)} 15-min intervals...")
    ...
    print(f"H2 Results: WinRate={win_rate:.1f}%, Sharpe={sharpe:.2f}")
    ...
    plt.title(f"2025 H2 Simulation | Net PnL: €{df['cumulative_pnl'].iloc[-1]:,.2f} | Sharpe: {sharpe:.2f}")
    plt.xlabel("Date")
    plt.ylabel("PnL (€)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    
    out_chart = "output_2/simulation_h2_chart.png"
    ...
    # Matches H1 (which uses Stride 3 on Hourly data).
    WINDOW_SIZE = 72 
    STRIDE = 12
    ...
    out_gif = "output_2/simulation_h2.gif"
    if not os.path.exists("output_2"): os.makedirs("output_2")
    
    print(f"Saving H2 GIF to {out_gif}...")
